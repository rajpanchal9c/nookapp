...
// API Endpoint for Task Suggestion (Replicates Vercel Function)
app.post('/api/suggest-tasks', async (req, res) => {
    try {
        ...
        if (!apiKey) {
            ...
            return res.status(500).json({ error: 'AI service not configured (Check .env.local)' });
        }
...
Tasks (JSON array only):`;
        ...
        if (!responseText) {
            return res.status(500).json({ error: 'No response from AI', tasks: [] });
        }
        ...
        try {
            ...
            return res.status(500).json({ error: 'Failed to parse AI response', tasks: [] });
        }

        return res.status(200).json({ tasks });

    } catch (error) {
        console.error('Error in local suggest-tasks API:', error);
        return res.status(500).json({
...
app.listen(PORT, () => {
    ...
    console.log(`✨ AI Feature is enabled for local testing`);
});
